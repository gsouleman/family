...
async function testAPI() {
    ...
    try {
        ...
        if (!assetRes.ok) throw new Error(`Asset creation failed: ${assetRes.status} ${assetRes.statusText} - ${await assetRes.text()}`);
        ...
        const heirRes = await fetch(`${BASE_URL}/heirs`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                name: 'API Test Heir',
        ...
        if (!heirRes.ok) throw new Error(`Heir creation failed: ${heirRes.status} ${heirRes.statusText}`);
        const heir = await heirRes.json() as any;
